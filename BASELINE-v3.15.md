# BASELINE v3.15 — Inspection Entry Point

## Architecture summary
AI-HPP v3.15 is organized as an ISO/IEC-style policy library with a frozen core normative layer and supporting alignment, validation, and evidence tooling. Normative requirements live in `standard/`, supporting threat/incident rationale in `annex/`, and machine-readable evidence contracts in `schemas/`.
...
- Baseline marker version: v3.15.

## Governance status
- Core Normative Layer is frozen at v3.15 (`standard/`, `annex/`, `schemas/`).
- New baseline-affecting content requires threat reference, incident reference, and measurable verification logic.
- Legacy root-level `v2/` and `v3/` trees are archived under `archive/root-legacy/`.
